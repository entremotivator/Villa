...
def render_interactive_calendar_grid(manager, workbook):
    ...
    with col1:
        if st.button("◀️ Previous Month"):
            st.session_state.calendar_view_date -= timedelta(days=30)
            st.rerun()
    ...
    bookings_df = manager.get_bookings_by_date_range(workbook, start_date, end_date)
            ...
            with week_cols[i]:
                if day == 0:
                    ...
                    if not bookings_df.empty:
                        date_cols = [col for col in bookings_df.columns if 'date' in col.lower()]
                        if date_cols:
                            bookings_df[date_cols[0]] = pd.to_datetime(bookings_df[date_cols[0]], errors='coerce')
                            day_bookings = len(bookings_df[bookings_df[date_cols[0]].dt.date == day_date])
                    ...
                    '>
                        ...
                        <div style='font-size: 0.8rem; color: {text_color}; margin-top: 5px;'>
                            {day_bookings} booking{'s' if day_bookings != 1 else ''}
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
    ...
    if 'date' in st.session_state.quick_filters:
        ...
        if not bookings_df.empty:
            ...
            if date_cols:
                bookings_df[date_cols[0]] = pd.to_datetime(bookings_df[date_cols[0]], errors='coerce')
                day_bookings_df = bookings_df[bookings_df[date_cols[0]].dt.date == filtered_date]
...
def main_app():
    ...
    view_mode = st.radio(
        "📑 View Mode",
        ["Dashboard", "Create Full Booking", "Interactive Calendar", "Advanced Bulk Editor", 
         "Live Editor", "Calendar View", "Email Center", "Search & Analytics", 
         "Templates", "Backup & Restore", "Edit History", "System Logs"],
    ...
    elif view_mode == "Create Full Booking":
        if not st.session_state.current_workbook:
            st.info("👈 Please select a workbook from the sidebar")
            return
        workbook = manager.open_workbook(st.session_state.current_workbook)
        if workbook:
            render_full_booking_creator(manager, workbook)
    
    elif view_mode == "Interactive Calendar":
        if not st.session_state.current_workbook:
            st.info("👈 Please select a workbook from the sidebar")
            return
        workbook = manager.open_workbook(st.session_state.current_workbook)
        if workbook:
            render_interactive_calendar_grid(manager, workbook)
    
    elif view_mode == "Advanced Bulk Editor":
        if not st.session_state.current_workbook:
            st.info("👈 Please select a workbook from the sidebar")
            return
        workbook = manager.open_workbook(st.session_state.current_workbook)
        if workbook:
            render_advanced_bulk_editor(manager, workbook)
